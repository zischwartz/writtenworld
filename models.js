...
(function() {
  var CellSchema, FeedbackSchema, ObjectId, Rite, RiteSchema, Schema, UserSchema, WorldSchema, asyncReal, clone, events, findEdits, findEditsRecurse, makeSeedQs, mongoose, mongooseAuth, slugGenerator, util;
  ...
  makeSeedQs = function(seed) {
    var down, left, result, right, up;
    if (!seed) {
      console.log('NOT SEED in makeseedQs');
    }
    up = {
      world: seed.world,
      x: seed.x,
  ...
  findEditsRecurse = function(seed, edit, dir) {
    var qs;
    console.log('...');
    console.log('recuSeed: ', seed);
    qs = makeSeedQs(seed);
    console.log('recurse, for ', qs[dir]);
    return exports.Cell.findOne(qs2[dir]).populate('current').run(function(err, cell) {
      if (cell != null ? cell.current : void 0) {
        console.log(cell);
        console.log('r: ', cell.current.contents);
        return findEditsRecurse(qs2[dir], edit, dir);
      }
    });
  };

  findEdits = function(seed, edit, callback) {
    var i, qs, qs2, _i, _ref, _results;
    qs = makeSeedQs(seed);
    _results = [];
    for (i = _i = 0, _ref = qs.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
      console.log(qs[i]);
      qs2 = clone(qs);
      _results.push(exports.Cell.findOne(qs[i]).populate('current').run(function(err, cell) {
        if (cell != null ? cell.current : void 0) {
          console.log('c: ', cell != null ? cell.current.contents : void 0);
          console.log('qs2[i]: ', qs2[i]);
          return findEditsRecurse(clone(qs2[i], edit, i));
        }
      }));
    }
    return _results;
