(function() {
  ...
  module.exports = function(app, SessionModel) {
    ...
    everyone.now.writeCell = function(cellPoint, content) {
      ...
      if (this.user.session.auth) {
        ...
        SessionModel.findOne({
          ...
          return models.writeCellToDb(cellPoint, content, models.mainWorldId, ownerId, isOwnerAuth, props);
        });
      }
    ...
    everyone.now.getTile = function(absTilePoint, numRows, callback) {
      var _this = this;
      return models.Cell.where('world', models.mainWorldId).where('x').gte(absTilePoint.x).lt(absTilePoint.x + numRows).where('y').gte(absTilePoint.y).lt(absTilePoint.y + numRows).populate('current').run(function(err, docs) {
